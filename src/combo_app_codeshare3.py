...
class FlightDataCollector:
    ...
    def categorize_flight(self, time_str):
        """Categorize flight with better error handling"""
        if not time_str:
            return "Unknown"
        ...
        try:
            ...
            if 'T' in time_str:
                ...
                # Try parsing as simple time format
    ...
    def process_codeshare_flights(self, flights, debug_codeshares=True):
        ...
        for flight in flights:
            ...
            is_arrival = 'arr_icao' in flight.get('_query', {}) if '_query' in flight else True
            time_field = flight.get('arrival' if is_arrival else 'departure', {}).get('actual')
            if not time_field:
                time_field = flight.get('arrival' if is_arrival else 'departure', {}).get('scheduled')
