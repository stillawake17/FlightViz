...
class FlightDataCollector:
    ...
    def fetch_flights(self, date, is_arrival=True, limit=100):
        """
    ...
    def categorize_flight(self, time_str):
        """
        Categorize flight based on its actual time
        """
        if not time_str:
            return "Unknown"
    ...
    def process_codeshare_flights(self, flights):
        ...
        Only includes flights with status 'landed'
        """
        processed_flights = []
        primary_flight_numbers = set()
        
        for flight in flights:
            # Skip flights that haven't landed
            if flight.get('flight_status') != 'landed':
                continue
                
            # Check if this is a codeshare flight
            codeshared = flight.get('flight', {}).get('codeshared')
            flight_number = flight.get('flight', {}).get('number')
            
            # Skip if this is a codeshare and we've already processed the primary flight
            if codeshared and codeshared.get('flight_number') in primary_flight_numbers:
                continue
                
            # Add flight number to processed set
            if flight_number:
                primary_flight_numbers.add(flight_number)
                
            # Add time category to flight data
    ...
    def collect_daily_data(self, date_str=None):
        ...
        # Process to handle codeshares and filter for landed flights only
        processed_arrivals = self.process_codeshare_flights(arrivals)
        processed_departures = self.process_codeshare_flights(departures)
        
        print(f"Total arrivals retrieved: {len(arrivals)}")
        print(f"Landed arrivals after processing: {len(processed_arrivals)}")
        print(f"Total departures retrieved: {len(departures)}")
        print(f"Landed departures after processing: {len(processed_departures)}")
        ...
        # Save file
        filename = f"{self.data_dir}/{self.airport_code}_landed_{date_str}.json"
    ...
    def generate_summary(self, data):
        """
        Generate summary statistics for landed flights data
        """
        # Categories to count
        categories = {
            'Regular flights': 0,
        ...
        # Create summary
        summary = {
            'total_landed_flights': len(data['arrivals']) + len(data['departures']),
            'landed_arrivals': len(data['arrivals']),
            'landed_departures': len(data['departures']),
