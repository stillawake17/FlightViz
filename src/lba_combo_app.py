...
yesterday = (datetime.now() - timedelta(days=22)).strftime("%Y-%m-%d")

# Fetch and save arrival data
arrival_filename = f'LBA_arrivals_{yesterday}.json'
fetch_data(yesterday, is_arrival=True, filename=arrival_filename)

# Fetch and save departure data
departure_filename = f'LBA_departures_{yesterday}.json'
...
arrival_filename = f'LBA_arrivals_{yesterday}.json'
departure_filename = f'LBA_departures_{yesterday}.json'
combined_filename = f'LBA_combined_{yesterday}.json'
...
input_file_path = f'LBA_combined_{yesterday}.json'
...
file1_path = 'data\LBAcombined_strip.json'
...
if data1 is not None and data2 is not None:
    ...
    output_path = 'data/LBAcombined_strip.json'
